...
use Haruncpi\LaravelIdGenerator\IdGenerator as LaravelIdGeneratorIdGenerator;
use Illuminate\Support\Facades\DB, Exception;

class IdGenerator extends LaravelIdGeneratorIdGenerator
{
    ...
    public static function generate($configArr)
    {
        ...
        if ($total[0]->total) {
            ...
            // BUG MAXID FIXED FOR MY PROJECT
            if($field != 'id') {
                if ($resetOnPrefixChange) {
                    $maxQuery = sprintf("SELECT %s AS maxid FROM %s WHERE %s AND id IN (SELECT MAX(id) FROM %s) LIKE %s", $field, $table, $field, $table, "'" . $prefix . "%'");
                } else {
                    $maxQuery = sprintf("SELECT %s AS maxid FROM %s WHERE id IN (SELECT MAX(id) FROM %s)", $field, $table, $table);
                }
            }
